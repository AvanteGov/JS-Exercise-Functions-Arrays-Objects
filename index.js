...
// const the3rd = function inventory.find(item, index){
//   return index === 2
// }
...
function getCarInfoByIndex(inventory, index) {
  const arrayIndex = inventory.find((item, index) => {
    return index === index})
    
    return `This is a ${inventory[index].car_make} ${inventory[index].car_model}`;
  }
                // array > object in the array (by index) > value data of the key
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inventoryArray, idNum) {
  const idFinder = inventoryArray.find((item, index) => {
    return index === idNum - 1
  })
  return `This is a ${inventoryArray[idNum - 1].car_make} ${inventoryArray[idNum - 1].car_model}`
}
...
*/
function sortCarInventory(unsortedArray) {
  const copyOfInventory = unsortedArray.sort(function(a ,b){
...
function getModelYears(array) {
  const carYears = array.map(c => c.car_year); 
                                            // where is the parameter c coming into play??? 
  return carYears;
}
...
function getOlderCars(inventoryArray, maxYear) {
  const cars = inventoryArray.filter(c => c.car_year <= maxYear);
// Need clarification on arrow functions.
  return cars;
}
...
// this test script makes no sense 
